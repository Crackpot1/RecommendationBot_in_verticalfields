...
documents = loop.run_until_complete(load_and_chunk_pdfs(pdf_files, chunk_size=1000))

# Initialize the generator
generator_llm = Ollama(model="deepseek-r1:1.5b")
critic_llm = Ollama(model="deepseek-r1:1.5b")
...
test_samples = generator.generate_with_langchain_docs(documents, test_size=10, distributions={simple: 0.5, reasoning: 0.25, multi_context: 0.25})

# Print structure of test_samples for debugging
print(test_samples)
...
# Display the synthetic test set
st.write("Synthetic Test Set")
st.dataframe(df)
