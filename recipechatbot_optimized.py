...
VECTOR_CACHE_FILE = CACHE_DIR / "vector_store.pkl"
EMBEDDING_CACHE_FILE = CACHE_DIR / "embeddings.pkl"

# 确保缓存目录存在
CACHE_DIR.mkdir(exist_ok=True)

def get_pdf_hash(pdf_files):
    """计算PDF文件的哈希值，用于缓存验证"""
    hash_content = ""
    for pdf_file in sorted(pdf_files):
        stat = os.stat(pdf_file)
        hash_content += f"{pdf_file}:{stat.st_mtime}:{stat.st_size}"
    return hashlib.md5(hash_content.encode()).hexdigest()

def save_vector_cache(vector_store, pdf_hash):
    """保存向量库到缓存"""
    try:
        cache_data = {
            "vector_store": vector_store,
            "pdf_hash": pdf_hash,
            "timestamp": os.path.getmtime("Recipe material")
        }
...
def load_vector_cache(pdf_hash):
    """从缓存加载向量库"""
    try:
        ...
        # 验证缓存是否有效
        if (cache_data["pdf_hash"] == pdf_hash and 
            cache_data["timestamp"] == os.path.getmtime("Recipe material")):
            print("从缓存加载向量库")
            return cache_data["vector_store"]
        else:
            print("缓存已过期，需要重新生成")
            return None
    except Exception as e:
...
# 优化的PDF加载和分块
async def load_and_chunk_pdfs_optimized(pdf_paths, chunk_size):
    print(f"正在处理 {len(pdf_paths)} 个PDF文件...")
    ...
    print("PDF文本提取完成，正在分块...")
    
    documents = [Document(page_content=text) for text in texts]
    text_splitter = RecursiveCharacterTextSplitter(
        chunk_size=chunk_size, 
        chunk_overlap=200,  # 增加重叠以保持上下文
        length_function=len,
        separators=["\n\n", "\n", "。", "！", "？", ".", "!", "?"]
    )
...
# 优化的向量库创建
def create_vector_store_optimized(split_docs):
    print("正在创建向量库...")
    try:
...
def initialize_system():
    ...
    # 计算PDF文件哈希值
    pdf_hash = get_pdf_hash(pdf_files)
    
    # 尝试从缓存加载向量库
    global vector_store
    vector_store = load_vector_cache(pdf_hash)
    
    if vector_store is None:
        print("缓存未找到或已过期，正在重新生成向量库...")
        # 异步加载和分块PDF文档
        try:
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            split_docs = loop.run_until_complete(load_and_chunk_pdfs_optimized(pdf_files, chunk_size=1000))
            vector_store = create_vector_store_optimized(split_docs)
            
            if vector_store:
                # 保存到缓存
                save_vector_cache(vector_store, pdf_hash)
            else:
                print("向量库创建失败")
                return False
